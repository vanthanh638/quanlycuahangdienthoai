...
public class AdminQuangCaoController {
	...
	// addItem
	public String add(@Valid @ModelAttribute("quangCao") QuangCao quangCao, BindingResult bindingResult,
			@RequestParam("picture_add") CommonsMultipartFile cmf, HttpServletRequest request) {
		...
		if (qc != null) {
			if (quangCaoDao.delItem(id) > 0) {
				if (!"".equals(qc.getHinhanh())) {
					String urlFile = request.getServletContext().getRealPath("files") + File.separator
							+ qc.getHinhanh();
	...
	public String edit(@Valid @ModelAttribute("objQC") QuangCao objQC, BindingResult bindingResult,
			@PathVariable("id") int id, @RequestParam("picture_add") CommonsMultipartFile cmf,
			HttpServletRequest request, @RequestParam(value = "page", defaultValue = "1") int page) {
